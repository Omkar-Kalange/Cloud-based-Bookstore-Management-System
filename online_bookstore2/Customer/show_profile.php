<?php
    ...
    {
        move_uploaded_file($_FILES["profPic"]["tmp_name"],"i.png");
        $s3->putObject([
            'Bucket'=>'onlinebookstoreservice',            //Bucket where to put image
            'Key' => 'ProfilePics/Customer/'.$custID.'.png', //Name of the file in the bucket
            'Body' =>fopen('i.png','rb')            //File can be viewed on S3
        ]);
        unlink("i.png");

        $query = "UPDATE customers SET profilePic = 'Y' WHERE customerID = ".$custID;
        try
        {
...
<!DOCTYPE html>
                ...
                <td>
                    <div class="main">
                        ...
                        <table class="profile">
                            ...
                            <tr>
                                <td colspan="2">
                                    <button class='signOut' onclick= 'return signOut(<?php echo $custID ?>)'>Sign out</button>
                                </td>
                            </tr>
                        </table>
                    </div>
    ...
    <script>
        function signOut(ID)
        {
            if(confirm("Signing-out will delete your login access and access to purchased e-books. Are you sure you want to sign-out?"))
                {
